...
exports.admin_activation = function(req, res, next) {
  ...
  // See if an admin with the given email exists
  User.findOne({ email: email }, function(err, existingUser) {
    if (err) { return next(err); }

    // If an admin with email does exist, return an error
    if (existingUser) {
      return res.status(422).send({ error: 'Email is in use' });
    }

    // If a user with email does NOT exist, create and save admin record
    const user = new User({
      email: email,
      password: password,
      role: 'admin'
